...
case class Address(
  ...
  lazy val fullAddress: Seq[String] = List(line1, line2, line3, line4, line5, postcode.map(_.toUpperCase), internationalAddressCountry(country)).flatten
  ...
  def internationalAddressCountry(country: Option[String]): Option[String] =
    if(excludedCountries.contains(Country(country.getOrElse("")))) None else country
...
object Address extends Logging with LocalDateFormat {

  implicit val writes: Writes[Address] = (address: Address) => removeNulls(
    Json.obj(
      "line1" -> address.line1,
      "line2" -> address.line2,
      "line3" -> address.line3,
